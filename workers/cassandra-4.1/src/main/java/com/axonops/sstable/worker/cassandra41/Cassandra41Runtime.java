...
public final class Cassandra41Runtime implements ImportRuntimeAdapter {
    ...
    private static void requireIsolationProperties(Path configurationFile) {
        ...
        if (configured == null) throw new IllegalStateException("cassandra.config is required");
        try {
            if (!Paths.get(URI.create(configured)).toRealPath()
                    .equals(configurationFile.toRealPath())) {
                throw new IllegalStateException("cassandra.config does not reference the workspace configuration");
            }
    ...
    private static void validateConfiguration(Path root) throws IOException {
        ...
        requireOwnedPath(root, Paths.get(DatabaseDescriptor.getCommitLogLocation()), "commitlog_directory");
        requireOwnedPath(root, Paths.get(DatabaseDescriptor.getSavedCachesLocation()), "saved_caches_directory");
        requireOwnedPath(root, DatabaseDescriptor.getHintsDirectory().toPath(), "hints_directory");
        requireOwnedPath(root, Paths.get(DatabaseDescriptor.getCDCLogLocation()), "cdc_raw_directory");
    }
